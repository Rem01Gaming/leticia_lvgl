...
/**
 * @brief Invalidates every root layer of the display, not just the active screen.
 *
 * The active screen is only one of several independent object trees LVGL
 * composites together (screen, top layer, sys layer, bottom layer). Any
 * persistent overlay -- the status bar, a future toast/dialog on the top
 * layer, etc. -- lives outside the active screen and won't be redrawn on
 * wake unless its own root is invalidated too. Sleep also physically zeroes
 * the framebuffer (see blank_framebuffer_pixels()), so anything not
 * invalidated here stays visibly blank after waking even though LVGL still
 * thinks it's showing.
 *
 * Invalidating these four fixed roots covers whatever is attached to them
 * without power_manager needing to know what those objects are, so new
 * views or overlays added later don't require a change here.
 */
    void power_manager::invalidate_all_layers() {
        if (disp_ == nullptr)
            return;

        lv_obj_invalidate(lv_display_get_screen_active(disp_));
        lv_obj_invalidate(lv_display_get_layer_top(disp_));
        lv_obj_invalidate(lv_display_get_layer_sys(disp_));
        lv_obj_invalidate(lv_display_get_layer_bottom(disp_));
    }
