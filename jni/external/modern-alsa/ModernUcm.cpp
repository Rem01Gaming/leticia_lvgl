...
/**
 * @brief Drives one directive against the control it names, dispatching on
 * the control's actual kernel element type.
 * @return false only on a hard I/O failure; true if the write succeeded or
 *         the control doesn't exist on this card (skipped, not fatal).
 */
bool apply_one(const ModernAlsa::mixer &mx, const directive &d, int *out_error) noexcept {
    ...
    if (ctl->is_boolean()) {
        bool on = (d.kind == directive_kind::disable) ? false
                : (d.val_kind == value_kind::integer)  ? (d.int_value != 0)
                                                         : true;
        ...
        if (d.val_kind == value_kind::text) {
            ...
            unsigned int item = (d.kind == directive_kind::disable) ? 0
                               : (d.val_kind == value_kind::integer) ? static_cast<unsigned int>(d.int_value)
                                                                       : 1;
            r = ctl->set_all_enum_indices(item);
        }
    } else if (ctl->is_integer()) {
        long v = (d.kind == directive_kind::disable) ? 0
                : (d.val_kind == value_kind::integer)  ? d.int_value
                                                         : 1;
...
bool apply_sequence(const ModernAlsa::mixer &mx, const directive *directives, size_type count,
                     int *out_error) noexcept {
